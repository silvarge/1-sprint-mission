...
public class UserValidator implements Validator<User, UserSignupRequestDto, UserUpdateDto> {
  ...
  public User validateUpdate(User current, UserUpdateDto update) {
    ...
    if (update.nickname() != null && !update.nickname().equals(current.getNickname())
        && ValidatorExp.NICKNAME.matches(update.nickname())) {
      current.updateNickname(update.nickname());
      isUpdated = true;
    }

    if (update.email() != null && !update.email().equals(current.getEmail())
        && ValidatorExp.EMAIL.matches(update.email())) {
      current.updateEmail(update.email());
      isUpdated = true;
    }

    if (update.role() != null && (update.role() != Role.fromString(
        current.getRole().toString().toUpperCase()))) {
      current.updateRole(Role.fromString(update.role().toString().toUpperCase()));
      isUpdated = true;
    }

    // phone
    if ((update.regionCode() != null && update.phone() != null)
        && (!update.phone().equals(current.getPhone().getPhone())
        || update.regionCode() != Phone.RegionCode.fromString(
        current.getPhone().getRegionCode().toString().toUpperCase()))
