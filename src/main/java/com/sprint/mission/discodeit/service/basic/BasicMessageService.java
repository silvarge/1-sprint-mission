...
public class BasicMessageService implements MessageService {
  ...
  public PageResponse<MessageResponseDto> findMessagesByChannelId(UUID channelId, UUID cursor) {
    ...
    UUID nextCursor =
        messages.hasNext() ? messages.getContent().get(messages.getNumberOfElements() - 1).getId()
            : null;
  ...
  @Transactional
  @Override
  public MessageResponseDto update(UUID messageId, String content, List<MultipartFile> attachments)
      throws IOException {
    ...
    if (attachments != null && !attachments.isEmpty()) {
      ...
      if (!message.getAttachments().isEmpty()) {
        ...
        log.info("메시지에 첨부파일이 추가되었습니다. - messageId: {}, 첨부파일 수: {}", message.getId(),
            attachments.size());
      }
      addAttachmentToMessage(message, attachments);
    }
