...
public class BasicAuthService implements AuthService {
    ...
    @Override
    public UserResponseDto login(AuthDTO.loginReq loginDTO) {
        if (userRepository.confirmLogin(loginDTO.username(), loginDTO.password())) {
            ...
            Map.Entry<Long, UserStatus> userStatus = userStatusRepository.findUserStatusByUserId(user.getValue().getPublicId());

            // 로그인 했으니 시간 업데이트
            userStatus.getValue().updateAccessedAt();
            userStatusRepository.update(userStatus.getKey(), userStatus.getValue());

            return UserResponseDto.from(user.getValue(), userStatus.getValue().isOnline());
        }
